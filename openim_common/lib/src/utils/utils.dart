...
class IMUtils {
  ...
  static Future<File> getVideoThumbnail(File file) async {
    ...
    try {
      final thumbnailFile = await VideoCompress.getFileThumbnail(
        path,
        quality: 50,
        position: -1, // 使用默认位置（第一帧）
      );
      
      // 复制到目标路径
      if (thumbnailFile.existsSync()) {
        thumbnailFile.copySync(targetPath);
        return File(targetPath);
      }
    } catch (e) {
      Logger().printError(info: "获取视频缩略图失败: $e");
    }
  ...
  static Future<File?> compressVideoAndGetFile(File file) async {
    ...
    try {
      ...
      if (info?.file != null) {
        info!.file!.copySync(targetPath);
        return File(targetPath);
      }
    } catch (e) {
      Logger().printError(info: "视频压缩失败: $e");
    }
  ...
  /// 安全的 trim 方法
  static String safeTrim(String? text) => text?.trim() ?? '';

  /// 时间格式1: yyyy-MM-dd
  static String getTimeFormat1() => 'yyyy-MM-dd';

  /// 时间格式2: yyyy-MM-dd HH:mm
  static String getTimeFormat2() => 'yyyy-MM-dd HH:mm';

  /// 时间格式3: yyyy-MM-dd HH:mm:ss
  static String getTimeFormat3() => 'yyyy-MM-dd HH:mm:ss';
}
