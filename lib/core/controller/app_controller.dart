...
class AppController extends SuperController with UpgradeManger {
  ...
  void onInit() async {
    ...
    // Android 后台权限引导（延迟显示，避免与其他权限请求冲突）
    Future.delayed(const Duration(seconds: 3), () {
      AndroidBackgroundHelper.checkAndShowGuides();
    });
  ...
  Future<void> showNotification(
    ...
    Logger.print(
      'showNotification called: contentType=${message.contentType}, sendID=${message.sendID}, showNotification=$showNotification',
    );
    ...
    if (_isGlobalNotDisturb() ||
      ...
      Logger.print(
        'showNotification skipped: globalNotDisturb=${_isGlobalNotDisturb()}, notPush=${message.attachedInfoElem?.notSenderNotificationPush}, isTyping=${message.contentType == im.MessageType.typing}, isSelf=${message.sendID == OpenIM.iMManager.userID}',
      );
      return;
    }
    ...
    if (sourceID != null && message.sessionType != null) {
      ...
      Logger.print('showNotification: conversation recvMsgOpt=${i.recvMsgOpt}');
      if (i.recvMsgOpt != 0) return;
    }

    if (showNotification) {
      Logger.print('showNotification: calling promptSoundOrNotification');
      promptSoundOrNotification(message.seq!);
    } else {
      Logger.print('showNotification: showNotification=false, skipped');
    }
  }

  Future<void> promptSoundOrNotification(int seq) async {
    Logger.print(
      'promptSoundOrNotification: isRunningBackground=$isRunningBackground',
    );
  ...
  void _playMessageSound() async {
    if (shouldMuted) {
      Logger.print('_playMessageSound: shouldMuted=true, skip');
      return;
    }
    bool isRegistered = Get.isRegistered<IMController>();
    ...
    if (isRegistered) {
      ...
      Logger.print(
        '_playMessageSound: allowVibration=${logic.userInfo.value.allowVibration}, allowBeep=${logic.userInfo.value.allowBeep}',
      );
    }
    // 获取系统静音、震动状态
    RingerModeStatus ringerStatus = await SoundMode.ringerModeStatus;
    Logger.print(
      '_playMessageSound: ringerStatus=$ringerStatus, isAllowBeep=$isAllowBeep, isPlaying=${_audioPlayer.playerState.playing}',
    );
