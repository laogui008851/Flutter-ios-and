...
class SetPasswordLogic extends GetxController {
  ...
  bool _checkingInput() {
    ...
    final nicknameRegex = RegExp(r'^[a-zA-Z0-9]{9}$');
    if (!nicknameRegex.hasMatch(nickname)) {
      IMViews.showToast(StrRes.wrongNicknameFormat);
      return false;
    }
  ...
  void register() async {
    await LoadingView.singleton.wrap(asyncFunction: () async {
      final data = await Apis.register(
        ...
        account: phoneNumber, // phoneNumber 字段存储的是用户名
        email: email,
      ...
      final account = {"username": phoneNumber};
